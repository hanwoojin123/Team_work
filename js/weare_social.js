window.addEventListener("load", function(){
	...
	function sticky_func(_targetWrap){
		...
		const $btn_wrap = document.querySelector($sticy_sec +" > div.btns_wrap");
	...
	function popup_active(_moreBtn_Wrap, _closeBtn_wrap){

		const $sec_wrap = _moreBtn_Wrap;
		const $pop_wrap = _closeBtn_wrap;

		const $more_btns = document.querySelectorAll($sec_wrap +" .div_all .btn_more");
		const $pop_bg = document.querySelectorAll($pop_wrap +" .popup_bg");
		const $close_btns = document.querySelectorAll($pop_wrap +" .close_btn");


		for(var i = 0; i < $more_btns.length; i++){
			$more_btns[i].index = i;
			$more_btns[i].onclick = function(){
				$pop_bg[this.index].style.display = "block";
				$body.style.overflow = "hidden";
			}
		}

		for(var i = 0; i < $close_btns.length; i++){
			$close_btns[i].index = i;
			$close_btns[i].onclick = function(){
				$pop_bg[this.index].style.display = "none";
	...
	if(!isMobile){
		...
		let tab_show_01 = SlideShow_Tabs("section.social_con", true)
		let sticky_func_01 = sticky_func("section.social_con");
		let span_blind_01 = blind("section.social_con", "h3");
		...
		if(screen.width >= 768){
			...
			const $blind_con = document.querySelector("section.social_con .div_all")
			
			$blind_con.classList.remove("blind");

			let popup_01 = popup_active("section.social_con", "div.popup_box");
